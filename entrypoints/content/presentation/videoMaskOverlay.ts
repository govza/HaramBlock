 ...
 * Implements IMediaOverlayModuleAPI<HTMLVideoElement>
 */
class VideoMaskOverlay implements IMediaOverlay<HTMLVideoElement> {
  ...
  async createMaskOverlay(
    ...
    if (allMasks.length > 0) {
      ...
      const state = this.createVideoOverlayElement(
        video,
        allMasks,
        ...
        posterImage,
        // Only override source dimensions when using poster; for video frames rely on video natural dims
      ...
      // No masks found, no overlay needed
    }
  }
  ...
  private createVideoOverlayElement(
    video: HTMLVideoElement,
    allMasks: { masks: number[][]; boundingBox: { x: number; y: number; width: number; height: number } }[],
    maskTransform: IMaskTransform,
    originalWidth: number, // Inference dimensions (from model)
    originalHeight: number,
    posterImage?: HTMLImageElement,
    sourceWidth?: number, // Element dimensions (poster or video) for layout/texture
    sourceHeight?: number,
  ): IMediaOverlayState {
    const parent = video.parentElement;
    ...
    // Use source dimensions for layout calculation if provided
    ...
    // Use a high z-index to ensure overlay appears above other absolutely positioned elements
  ...
  private setupObservers(video: HTMLVideoElement, state: IMediaOverlayState): void {
    // Implementation similar to image observers but for videos
    // Simplified for now
    this.videoStates.set(video, state);
  }
  ...
  private updateVideoOverlay(_video: HTMLVideoElement, _state: IMediaOverlayState): void {
    ...
    const prediction = state.currentPrediction as IImagePrediction | undefined;
...
/**
 * Load poster image for video overlay
 */
async function loadPosterImage(posterUrl: string): Promise<HTMLImageElement> {
  return new Promise((resolve, reject) => {
    ...
    setTimeout(() => reject(new Error('Poster image load timeout')), 3000);
