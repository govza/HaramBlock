  ...
  const renderSvgPath = () => {
    switch (hostSettings.policy) {
      case 'whitelist':
        return (
          <>
            <title>{i18n.t('hostPolicyWhitelist')}</title>
            <path
              fill="white"
      ...
      case 'blacklist':
        return (
          <>
            <title>{i18n.t('hostPolicyBlacklist')}</title>
            <path
              fill="red"
      ...
      default:
        return (
          <>
            <title>{i18n.t('hostPolicyProcess')}</title>
            <path
              fill="green"
  ...
  return (
    <div className={`flex w-full items-center p-2 ${isGlobalSettings ? 'bg-red-900' : 'bg-gray-700'}`}>
      <p className="w-full truncate text-center text-lg">
        {isGlobalSettings ? i18n.t(defaultGlobalKey) : hostSettings.hostname}
      </p>
      <button onClick={togglePolicy} className="cursor-pointer">
        <svg className="inline-block h-[1.5em] w-auto fill-current pl-1" viewBox="0 0 24 24">
          {renderSvgPath()}
        </svg>
      </button>
    </div>
