...
const createMockPredictionCache = (overrides: Partial<IImagePrediction> = {}): MockPredictionCache => {
  ...
  describe("getCachedPredictionsByHostname", () => {
    describe("Complex Business Logic", () => {
      it("should handle multiple predictions with mixed validity states", async () => {
        ...
        mockFindValidByHostname.mockResolvedValue([validPrediction1, validPrediction2]);

        const result = await service.getCachedPredictionsByHostname(TEST_HOSTNAME);
      ...
      it("should update access times asynchronously without blocking response", async () => {
        ...
        const prediction2 = createMockPredictionCache({ src: "https://example.com/image2.jpg" });
        
        // Make save operations slow to test async behavior
        prediction1.save.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
        prediction2.save.mockImplementation(() => new Promise(resolve => setTimeout(resolve, 100)));
      ...
      it("should handle background save failures gracefully without affecting response", async () => {
        ...
        prediction.save.mockRejectedValue(new Error("Database connection failed"));
      ...
      it("should handle mixed success/failure in background save operations", async () => {
        ...
        const successPrediction = createMockPredictionCache({ src: "success.jpg" });
        ...
        mockFindValidByHostname.mockResolvedValue([successPrediction, failPrediction]);
      ...
      it("should handle database query failures appropriately", async () => {
        ...
        await expect(service.getCachedPredictionsByHostname(TEST_HOSTNAME))
      ...
      it("should validate hostname parameter correctly", async () => {
        // Test empty string
        await expect(service.getCachedPredictionsByHostname(""))
        ...
        await expect(service.getCachedPredictionsByHostname(null as unknown as string))
          .rejects.toThrow("Hostname is required");
          
        // Test undefined (cast to string as the method expects string)
        await expect(service.getCachedPredictionsByHostname(undefined as unknown as string))
          .rejects.toThrow("Hostname is required");
          ...
          predictions: [
            {
              ...
              polygon: [{ x: 50, y: 75 }, { x: 350, y: 75 }, { x: 350, y: 275 }, { x: 50, y: 275 }]
            }
          ],
