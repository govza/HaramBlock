...
export class InferenceService {
   ...
   */
  async processImages(imageSrcs: string[], hostname: string, tabId: number): Promise<void> {
    logger.withTag('inferenceService').debug(`Processing ${imageSrcs.length} images for hostname: ${hostname}`);
    
    try {
      // Process images in batches and send results as they complete
      await this.processBatchedInference(imageSrcs, hostname, tabId);
      
      logger.withTag('inferenceService').debug(`Successfully completed processing all ${imageSrcs.length} images`);
    } catch (error) {
      logger.withTag('inferenceService').error('Error processing images in inference service:', error);
      throw error;
    }
  }
   ...
   * @param tabId - Tab ID for sending results back to content script
   */
  private async processBatchedInference(imageSrcs: string[], hostname: string, tabId: number): Promise<void> {
    ...
    logger.withTag('inferenceService').debug(`Processing ${totalImages} images in ${totalBatches} batches of ${batchSize}`);
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      try {
        logger.withTag('inferenceService').debug(`Processing batch ${batchIndex + 1}/${totalBatches}: ${batchImageSrcs.length} images`);
        ...
        const batchPredictions = await this.generateMockPredictions(batchImageSrcs, hostname);
        
        // Cache the predictions
        await this.predictionCacheService.cachePredictions(batchPredictions, hostname);
        ...
        logger.withTag('inferenceService').debug(`Completed batch ${batchIndex + 1}/${totalBatches}: ${processedCount}/${totalImages} images processed`);
        
      } catch (error) {
        logger.withTag('inferenceService').error(`Error processing batch ${batchIndex + 1}:`, error);
  ...
  private async sendPredictionsToContent(
    ...
    try {
      ...
      logger.withTag('inferenceService').debug(`Sent ${predictions.length} predictions to content script (tab ${tabId})`);
    } catch (error) {
  ...
  private async simulateAiProcessing(imageCount: number): Promise<void> {
    ...
    logger.withTag('inferenceService').debug(`Simulating AI processing for ${processingTime}ms...`);
   ...
   */
  private async generateMockPredictions(imageSrcs: string[], hostname: string): Promise<IImagePrediction[]> {
   ...
   */
  private createMockPrediction(src: string, hostname: string): IImagePrediction {
    const now = Date.now();
    
    // Generate random mock predictions with different classes
    const mockClasses = ['person', 'vehicle', 'animal', 'object'];
    const randomClass = mockClasses[Math.floor(Math.random() * mockClasses.length)] || 'object';
    ...
    const bboxWidth = Math.floor(imageWidth * 0.2 + Math.random() * (imageWidth * 0.3));
    const bboxHeight = Math.floor(imageHeight * 0.2 + Math.random() * (imageHeight * 0.3));
   ...
   */
  async processBatch(requests: Array<{ imageSrcs: string[], hostname: string, tabId: number }>): Promise<void> {
    logger.withTag('inferenceService').debug(`Processing batch of ${requests.length} inference requests`);
    ...
    try {
      ...
      for (let i = 0; i < requests.length; i += batchSize) {
        ...
        const batchPromises = batch.map(request => 
          this.processImages(request.imageSrcs, request.hostname, request.tabId)
        );
        
        await Promise.all(batchPromises);
      }
      
      logger.withTag('inferenceService').debug(`Successfully processed batch of ${requests.length} requests`);
    } catch (error) {
      logger.withTag('inferenceService').error('Error processing inference batch:', error);
      throw error;
    }
  }
}
